...
import time, random, sys#导入需要的自带模块
...
def ARP_one(): #The middle man intercepts the packet with ARPPing. 中间人拦截数据包带ARPPing模块(内网用)。
...
def nmap_port_open_scan():#nmap扫描所有端口状态
...
while True:#喜闻乐见的主循环
    print("quit(0)")
    print("The middle man intercepts the packet with ARPPing.(1)")
    print("SYN flood(2)")
    print("All port status scans(3)")
    print("退出(0)")
    print("中间人拦截数据包带ARPPing模块(内网用)。(1)")
    print("SYN洪水(2)")
    print("所有端口状态扫描(3)")
    choose = input(">>>")#用户选择输入
    choose = int(choose)
